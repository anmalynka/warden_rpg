...
export const ResourcesPanel = ({ resources }) => (
...
export const LevelCircle = ({ level, xp, xpToNext }) => {
  const radius = 24;
  ...
  return (
    <div className="relative flex items-center justify-center w-16 h-16 pointer-events-auto flex-shrink-0">
      {/* Background Circle */}
      <div className="absolute inset-0 bg-[#f9f5f0] rounded-full shadow-[0_4px_0_0_#d1c4b9] border-2 border-[#e6ded5] aspect-square" />
...
export const HUDContent = ({ resources, level = 1, xp = 0, xpToNext = 100 }) => {
  return (
...
const HUD = ({ resources, level = 1, xp = 0, xpToNext = 100 }) => {
  return (
    <div className="fixed top-4 left-0 right-0 pointer-events-none flex justify-center items-start z-[3000] px-4">
      <HUDContent resources={resources} level={level} xp={xp} xpToNext={xpToNext} />
    </div>
  );
};
